        ...
        <table className="divide-y divide-gray-100/10 w-full text-white">
            <thead>
                <tr className="divide-x divide-gray-100/10">
                    ...
                    {run.sets.map((set, i) => (<th key={set._id} className="px-4 py-3.5 text-sm text-center font-semibold">{set.name}</th>))}
                </tr>
            </thead>
            <tbody className="divide-y divide-gray-100/10">
                {run.templates.map((tmpl, i) => {
                    ...
                    return <tr className="divide-x divide-gray-100/10">
                        ...
                        <td className="group relative whitespace-pre-line px-4 py-8 text-sm">
                            <GridCell 
                                key={tmpl._id} 
                                result={run.results[`${tmpl.versionId}_${set._id}`]}
                                setId={set._id} 
                                gridId={grid?._id!}
                                historyId={run._id} 
...
const GridCell = ({templateId, result, gridId, setId, historyId, versionId, variables, llm}: TGridCellProps) => {
